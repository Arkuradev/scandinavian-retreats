...
type HttpOptions = { signal?: AbortSignal; params?: Record<string, string | number | boolean> };
...
export async function getVenues(opts: HttpOptions = {}) {
    ...
    if (!res.ok) throw new Error(`Failed to fetch venues: ${res.status} ${res.statusText}`);
    return res.json();
    }
